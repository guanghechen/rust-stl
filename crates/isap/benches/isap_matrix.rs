...
fn build_layered_graph() -> Isap {
    ...
    for i in 0..LAYER_SIZE {
        isap.add_edge(last_base + i, sink, 4);
    }

    isap
}

fn build_sparse_random_graph() -> Isap {
    let n = 256usize;
    let source = 0usize;
    let sink = n - 1;
    ...
    for u in 0..(n - 1) {
        isap.add_edge(u, u + 1, rng.next_cap(8));
    }
    ...
    for u in 0..n {
        for _ in 0..3 {
            ...
            isap.add_edge(u, v, rng.next_cap(10));
        }
    }
...
fn build_dense_random_graph() -> Isap {
    let n = 96usize;
    let source = 0usize;
    let sink = n - 1;
    ...
    for u in 0..n {
        for v in 0..n {
            ...
            if rng.next_usize(100) < 15 {
                isap.add_edge(u, v, rng.next_cap(20));
            }
        }
    }
...
fn build_bipartite_graph() -> Isap {
    ...
    for u in 0..left {
        isap.add_edge(source, u, 8);
    }

    for u in 0..left {
        for _ in 0..8 {
            let v = left + rng.next_usize(right);
            isap.add_edge(u, v, rng.next_cap(6));
        }
    }

    for v in 0..right {
        isap.add_edge(left + v, sink, 8);
    }

    isap
}

fn build_grid_graph() -> Isap {
    let rows = 28usize;
    let cols = 28usize;
    ...
    for r in 0..rows {
        for c in 0..cols {
            let u = r * cols + c;
            if c + 1 < cols {
                isap.add_edge(u, u + 1, 2);
            }
            if r + 1 < rows {
                isap.add_edge(u, u + cols, 2);
            }
        }
    }
    ...
    for c in 0..cols {
        isap.add_edge(last_row + c, sink, 3);
    }
...
fn bench_maxflow_matrix(c: &mut Criterion) {
    let mut group = c.benchmark_group("isap/matrix_maxflow");
...
fn bench_cached_matrix(c: &mut Criterion) {
    let mut group = c.benchmark_group("isap/matrix_cached_maxflow");
...
criterion_group!(isap_matrix, bench_maxflow_matrix, bench_cached_matrix);
criterion_main!(isap_matrix);
